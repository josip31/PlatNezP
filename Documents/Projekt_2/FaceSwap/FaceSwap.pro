...
UBUNTU_TRANSLATION_DOMAIN="faceswap.josip"
...
QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/FaceSwap/tests/autopilot/run
aptest.target   = autopilot
aptest.commands = QML2_IMPORT_PATH
aptest.depends  = sub-FaceSwap sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/FaceSwap/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-FaceSwap sub-backend-App
